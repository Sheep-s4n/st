...
bginit()
{
	...
	if (use_random_bg)
	{
		...
		srand(time(NULL)); // init the random number generator  with epoch as seed		

		struct dirent* entry; // a file 
		DIR* dp; // directory 
		int file_index = -1;

		dp = opendir(bgfolder);
		
		if (dp == NULL) // error when openning the dir
		{
			printf("%f\n" , "error while openning the directory");
			return;
		};



		while ( (entry = readdir(dp)) ) // return entry after assigning it to readdir(dp) --> loop breaks when readdir returns NULL = no more file to read
		{

			if (entry->d_type == DT_REG) // avoid printing "." and ".." files
			{	
				int length = strlen(entry->d_name); // get string length

				if (length >= 3)
				{	
					char extension[4]; //array of char to copy the file extension
					strncpy(extension , entry->d_name + length -3 , 3);
					extension[3] = '\0';// add null terminator to new string
					if (strcmp(extension , ".ff") == 0)
					{	
						file_index++;
						files_names[file_index] = entry->d_name; // adding file name to list
					};
				};	
			};
		};
